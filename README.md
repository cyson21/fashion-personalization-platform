...
| HTTP | FastAPI·uvicorn 선택 설치; 고정 문자열 `demo-admin` 로컬 관리자 토큰 확인 |
| 검증 방식 | core 단위 테스트, CLI 점검, 선택형 FastAPI import·route 등록 테스트 |
...
| FastAPI 어댑터 | 선택 의존성 없이 import 가능하며, 설치 시 주요 route가 등록됨 | [api.py](src/fashion_personalization/api.py), [test_api_adapter.py](tests/test_api_adapter.py) | HTTP 요청 검증·인증 실패·충돌 응답 테스트 없음 |
...
`demo-admin`은 로컬 시연용 고정 값입니다. 현재 자동 테스트는 어댑터 import와 route 등록까지만 확인하며 위 HTTP 요청 결과를 회귀 검증하지 않습니다.
...
- dead-letter 재처리는 서비스 메서드 수준이며 고정 관리자 토큰과 주요 HTTP 응답은 자동 회귀 테스트가 없습니다.
