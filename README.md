...
| 실행 환경 | Python 3.11 이상, CI는 Python 3.11·3.12, 기본 core는 표준 라이브러리, 테스트는 pytest 8 이상 |
...
JUnit XML과 CLI 결과를 만든 뒤 commit metadata를 고정하면 같은 테스트 판정과 claim metadata에서 byte-identical JSON 리포트를 생성할 수 있습니다. 실행 시간과 demo timestamp는 변동하므로 원본 진단 파일에만 남기고 최종 리포트에서는 제외합니다.

```bash
mkdir -p artifacts
python -m pytest -ra --junitxml=artifacts/pytest.xml
python -m fashion_personalization.cli > artifacts/cli-evidence.json

SOURCE_DATE_EPOCH="$(git show -s --format=%ct HEAD)" \
EVIDENCE_COMMIT="$(git rev-parse HEAD)" \
EVIDENCE_RUNTIME="python-$(python -c 'import sys; print(f"{sys.version_info.major}.{sys.version_info.minor}")')" \
python scripts/generate_portfolio_evidence.py \
  --junit artifacts/pytest.xml \
  --cli-json artifacts/cli-evidence.json \
  --output artifacts/portfolio-evidence.json
```

`portfolio-evidence.json`은 core·in-process HTTP·CLI에서 검증한 범위와 배포 서버·비동기 worker·영속 저장소·AWS·ML 품질처럼 검증하지 않은 범위를 분리합니다. CI는 Python 3.11과 3.12 각각에서 dependency compatibility, 전체 테스트, CLI, JSON 리포트를 검증하고 실패 여부와 관계없이 JUnit·설치 버전·runtime 진단 파일을 업로드합니다.
